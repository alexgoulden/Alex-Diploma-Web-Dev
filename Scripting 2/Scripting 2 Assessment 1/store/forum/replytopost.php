...
//check to see if showing form or adding post
if (!$_POST) {
	...
	//still need to ver topic and post
	...
	if (mysqli_num_rows($verify_res) < 1) {
		...
		while($topic_info = mysqli_fetch_array($verify_res)) {
			...
			$category_id = stripslashes($topic_info['category_id']);
		}
?>
<!DOCTYPE html>
<html>
...
<?php
	...
	//create safe values for use
	$safe_category_id = mysqli_real_escape_string($mysqli, $_POST['category_menu']);
	...
	$add_post_sql = "INSERT INTO forum_posts (topic_id, post_text, post_create_time, post_owner, category_id)
					 VALUES ('".$safe_topic_id."','".$safe_post_text."',now(),'".$safe_post_owner."', '".$safe_category_id."')";
