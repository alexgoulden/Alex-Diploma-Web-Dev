...
if((isset($_POST['firstname'])&& $_POST['firstname'] !='') && (isset($_POST['lastname'])&& $_POST['lastname'] !='') && (isset($_POST['birthdate'])&& $_POST['birthdate'] !='') && (isset($_POST['photo'])&& $_POST['photo'] !='') && (isset($_POST['notes'])&& $_POST['notes'] !='') && (isset($_POST['id']) && $_POST['id'] !=''));
{
	...
	$empid = $conn->real_escape_string($_POST['id']);
	...
	if(!ctype_alnum($fName) || !ctype_alnum($lName) || !ctype_alnum($p) || !ctype_alnum($n)) {
		...
		if ($errorsBirthdate['warning_count'] + $errorsBirthdate['error_count'] > 0) {
		   echo 'You did not follow the specified date format YYYY-MM-DD';
			...
			$sql = "CALL updateEmployee('$fName', '$lName', '$bDate', '$p', '$n', '$empid')";
